...
// FromGregorian returns a new Gregorian instance.
// 初始化 Gregorian 结构体
...
// FromLunar returns a new Lunar instance.
// 初始化 Lunar 结构体
...
func (g Gregorian) ToLunar() (l Lunar) {
	...
	l.timeZone = offset / 3600
	y, m, d := g.Date()
	l.day, l.month, l.year, l.isLeapYear, l.leapMonth, l.juliusDay = solarToLunar(d, m, y, l.timeZone)
...
func (l Lunar) ToGregorian() (g Gregorian) {
	...
	y, m, d := juliusDayToDate(l.juliusDay)
...
// IsCatYear reports whether is year of Cat.
...
func juliusDayToDate(jd int) (year, month, day int) {
...
func solarToLunar(dd, mm, yy, tz int) (lunarD, lunarM, lunarY, lunarLeap, lunarMonthLeap, juliusDayNumber int) {
	...
	newMoon := getNewMoonDay(k+1, tz)
	if newMoon > juliusDayNumber {
		newMoon = getNewMoonDay(k, tz)
	}
	...
	lunarD = juliusDayNumber - newMoon + 1
	diff := floor((intToFloat(newMoon) - intToFloat(a11)) / 29)
	...
	if b11-a11 > 365 {
		...
		if lunarMonthLeap < 0 {
			lunarMonthLeap += calendar.MonthsPerYear
		}
...
func lunarToJuliusDay(lunarDay, lunarMonth, lunarYear, lunarLeap, tz int) (juliusDay int) {
	var k, a11, b11, off, leapOff, leapMonth, newMoon int
	if lunarMonth < 11 {
	...
	off = lunarMonth - 11
	if off < 0 {
		off += calendar.MonthsPerYear
	}
	if b11-a11 > 365 {
		leapOff = getLeapMonthOffset(intToFloat(a11), tz)
		leapMonth = leapOff - 2
		if leapMonth < 0 {
			leapMonth += calendar.MonthsPerYear
		}
	...
	newMoon = getNewMoonDay(k+off, tz)
	juliusDay = newMoon + lunarDay - 1
	return
}
